...
export class UserSkillService {
  ...
  Get(id: number): Observable<any> {
    return this.httpClient.get<any>(`${environment.APIURL}/UserSkill/get/${id}`,
      { ...this.HttpOptions2 }
    ).pipe(catchError(this.handleError));
  }

  Create(Data: any) {
    return this.httpClient.post(`${environment.APIURL}/UserSkill`,
       JSON.stringify(Data), { ...this.HttpOptions2 }).pipe(catchError(this.handleError));
  
  }

  Edit(Data: any) {
    return this.httpClient.put(`${environment.APIURL}/UserSkill`,
       JSON.stringify(Data), { ...this.HttpOptions2 }).pipe(catchError(this.handleError));
  }
  ...
  Delete(id: number) {
    let params = new HttpParams()
      .set('id', id)
    return this.httpClient.delete<any>(`${environment.APIURL}/UserSkill`,
  ...
  GetToUser(userId:string){
    ...
    return this.httpClient.get<any>(
      `${environment.APIURL}/UserSkill/GetByUserId/${userId}`,
      {...this.HttpOptions2 }
    ).pipe(
