...
export class AcademicDegreeService {
  ...
  GetAll(page: number, pageSize: number): Observable<any> {
    ...
    return this.httpClient.get<any>(
      `${environment.APIURL}/AcademicDegree`,
  ...
  Get(id: number): Observable<any> {
    return this.httpClient.get<any>(`${environment.APIURL}/AcademicDegree/Get/${id}`,
      { ...this.HttpOptions2 }
    ).pipe(catchError(this.handleError));
  }

  Create(Data: any) {
    return this.httpClient.post(`${environment.APIURL}/AcademicDegree`,
       JSON.stringify(Data), { ...this.HttpOptions2 }).pipe(catchError(this.handleError));
  }

  Edit(Data: any) {
    return this.httpClient.put(`${environment.APIURL}/AcademicDegree`,
       JSON.stringify(Data), { ...this.HttpOptions2 }).pipe(catchError(this.handleError));
  ...
  Delete(id: number) {
    let params = new HttpParams()
      .set('id', id)
    return this.httpClient.delete<any>(`${environment.APIURL}/AcademicDegree/${id}`,
      { params, ...this.HttpOptions2 }
    ).pipe(catchError(this.handleError));;
  }
