...
export class SpecialityService {
  ...
  GetAll(page: number, pageSize: number, search: string): Observable<any> {
    ...
    return this.httpClient.get<any>(
      `${environment.APIURL}/Speciality/Specialites`,
  ...
  Get(id: number): Observable<any> {
    return this.httpClient.get<any>(`${environment.APIURL}/Speciality/get/${id}`,
      { ...this.HttpOptions2 }
    ).pipe(catchError(this.handleError));
  }

  Create(Data: any) {
    return this.httpClient.post(`${environment.APIURL}/Speciality/Create`,
       JSON.stringify(Data), { ...this.HttpOptions2}).pipe(catchError(this.handleError));
  }

  Edit(Data: any) {
    return this.httpClient.put(`${environment.APIURL}/Speciality/Edit`, 
      JSON.stringify(Data), { ...this.HttpOptions2 }).pipe(catchError(this.handleError));
  ...
  Delete(id: number) {
    let params = new HttpParams()
      .set('id', id)
    return this.httpClient.delete<any>(`${environment.APIURL}/Speciality/Delete/${id}`,
      { params, ...this.HttpOptions2}
    ).pipe(catchError(this.handleError));;
  }

  GetSpecialites() {
    return this.httpClient.get<any>(`${environment.APIURL}/Speciality/GetSpecialites`,
      { ...this.HttpOptions2 }
    ).pipe(catchError(this.handleError));
  }
}
