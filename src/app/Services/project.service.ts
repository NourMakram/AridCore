...
export class ProjectService {
  ...
  Get(id: number): Observable<any> {
    return this.httpClient.get<any>(`${environment.APIURL}/Project/${id}`,
      { ...this.HttpOptions2 }
    ).pipe(catchError(this.handleError));
  }
  
  GetProjectStuats(){ 
    return this.httpClient.get<any>(`${environment.APIURL}/Project/ProjectStuats`,
      { ...this.HttpOptions2}
    ).pipe(catchError(this.handleError));
  }

  Create(Data: any) {
    return this.httpClient.post(`${environment.APIURL}/Project`, 
      JSON.stringify(Data), { ...this.HttpOptions2 }).pipe(catchError(this.handleError));
  
  }

  Edit(Data: any) {
    return this.httpClient.put(`${environment.APIURL}/Project`, JSON.stringify(Data),
  ...
  GetToUser(userId:string){
    ...
    return this.httpClient.get<any>(
     `${environment.APIURL}/Project/GetByUserId/${userId}`,
      {...this.HttpOptions2 }
    ).pipe(
