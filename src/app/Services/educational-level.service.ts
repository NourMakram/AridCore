...
export class EducationalLevelService {
  ...
  Get(id: number): Observable<any> {
    return this.httpClient.get<any>(`${environment.APIURL}/EducationalLevel/${id}`,
      { ...this.HttpOptions2 }
    ).pipe(catchError(this.handleError));
  }
  Details(id: number): Observable<any> {
    return this.httpClient.get<any>(`${environment.APIURL}/EducationalLevel/Details/${id}`,
      { ...this.HttpOptions2 }
    ).pipe(catchError(this.handleError));
  }

  Create(Data: any) {
    return this.httpClient.post(`${environment.APIURL}/EducationalLevel`,
       JSON.stringify(Data), { ...this.HttpOptions2 }).pipe(catchError(this.handleError));
  
  }

  Edit(Data: any) {
    return this.httpClient.put(`${environment.APIURL}/EducationalLevel`,
       JSON.stringify(Data), { ...this.HttpOptions2 }).pipe(catchError(this.handleError));
  }
  ...
  Delete(id: number) {
    let params = new HttpParams()
      .set('id', id)
    return this.httpClient.delete<any>(`${environment.APIURL}/EducationalLevel/${id}`,
  ...
  GetToUser(userId:string){
    ...
    return this.httpClient.get<any>(
      `${environment.APIURL}/EducationalLevel/GetByUserId/${userId}`,
      {...this.HttpOptions2 }
    ).pipe(
