...
export class ExpertiseService {
  ...
  GetAll(page: number, pageSize: number): Observable<any> {
    ...
    return this.httpClient.get<any>(
      `${environment.APIURL}/Expertise/${page}/${pageSize}`,
  ...
  GetList(): Observable<any> {
    ...
    return this.httpClient.get<any>(
      `${environment.APIURL}/Expertise/List`,
      {...this.HttpOptions2 }
    ).pipe(
      catchError(this.handleError)
    );
  ...
  Get(id: number): Observable<any> {
    return this.httpClient.get<any>(`${environment.APIURL}/Expertise/get/${id}`,
      { ...this.HttpOptions2 }
    ).pipe(catchError(this.handleError));
  }

  Create(Data: any) {
    return this.httpClient.post(`${environment.APIURL}/Expertise`,
       JSON.stringify(Data), { ...this.HttpOptions2 }).pipe(catchError(this.handleError));
  
  }

  Edit(Data: any) {
    return this.httpClient.put(`${environment.APIURL}/Expertise`,
       JSON.stringify(Data), { ...this.HttpOptions2 }).pipe(catchError(this.handleError));
  }
  ...
  Delete(id: number) {
    let params = new HttpParams()
      .set('id', id)
    return this.httpClient.delete<any>(`${environment.APIURL}/Expertise`,
      { params, ...this.HttpOptions2}
    ).pipe(catchError(this.handleError));;
  }
