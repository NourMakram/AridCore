...
export class UserExpertiseService {
  ...
  Get(id: number): Observable<any> {
    return this.httpClient.get<any>(`${environment.APIURL}/UserExpertise/get/${id}`,
      { ...this.HttpOptions2 }
    ).pipe(catchError(this.handleError));
  }

  Create(Data: any) {
    return this.httpClient.post(`${environment.APIURL}/UserExpertise`, 
      JSON.stringify(Data), { ...this.HttpOptions2 }).pipe(catchError(this.handleError));
  
  }

  Edit(Data: any) {
    return this.httpClient.put(`${environment.APIURL}/UserExpertise`,
       JSON.stringify(Data), { ...this.HttpOptions2 }).pipe(catchError(this.handleError));
  }
  ...
  Delete(id: number) {
    let params = new HttpParams()
      .set('id', id)
    return this.httpClient.delete<any>(`${environment.APIURL}/UserExpertise`,
  ...
  GetToUser(userId:string){
    ...
    return this.httpClient.get<any>(
      `${environment.APIURL}/UserExpertise/GetByUserId/${userId}`,
      {...this.HttpOptions2 }
    ).pipe(
