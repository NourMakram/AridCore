...
HttpOptions2: { headers: HttpHeaders; };
  ...
  GetAll(page: number, pageSize: number): Observable<any> {
    ...
    return this.httpClient.get<any>(
      `${environment.APIURL}/Partner/GetAll/${page}/${pageSize}`,
      {...this.HttpOptions2 }
    ).pipe(
  ...
  Get(id: number): Observable<any> {
    return this.httpClient.get<any>(`${environment.APIURL}/Partner/Get/${id}`,
      { ...this.HttpOptions2 }
    ).pipe(catchError(this.handleError));
  }

  Create(Data: any) {
    return this.httpClient.post(`${environment.APIURL}/Partner`,
       JSON.stringify(Data), { ...this.HttpOptions2 }).pipe(catchError(this.handleError));
  
  }

  Edit(Data: any) {
    return this.httpClient.put(`${environment.APIURL}/Partner`,
       JSON.stringify(Data), { ...this.HttpOptions2 }).pipe(catchError(this.handleError));
  }

  Delete(id: number) {
    return this.httpClient.delete<any>(`${environment.APIURL}/Partner/${id}`,
      {...this.HttpOptions2}
    ).pipe(catchError(this.handleError));;
  }
