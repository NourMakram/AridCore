...
HttpOptions2: { headers: HttpHeaders; };
  ...
  GetAll(page: number, pageSize: number,keyword:string): Observable<any> {
    ...
    return this.httpClient.get<any>(
      `${environment.APIURL}/Statements/GetAll`,
  ...
  GetByUserId(userId:string,page:number,pageSize:number): Observable<any> {
       let params = new HttpParams();
    ...
    return this.httpClient.get<any>(
      `${environment.APIURL}/Statements/GetByUser/${userId}`,
      {params,...this.HttpOptions2 }
    ).pipe(
      catchError(this.handleError)
    );
  ...
  Create(Data: any) {
    return this.httpClient.post(`${environment.APIURL}/Statements`,JSON.stringify(Data),{ ...this.HttpOptions2 }).pipe(catchError(this.handleError));
  ...
  Delete(id: number) {
    return this.httpClient.delete<any>(`${environment.APIURL}/Statements/${id}`,
      {...this.HttpOptions2}
    ).pipe(catchError(this.handleError));;
  }
